...
def checkpoint_full_tree_reload(API, proc_list):
    ...
    API.context_switch([p for p in API.context.processes if p.p == 1][-1])
    ...
    for i, cmd in enumerate(cmd_list):
        if cmd['pid'] != API.context.self_proc:
            ...
            if ret is None or ret < 0:
                print(bcolors.WARNING+"Interpretation warning on:"+bcolors. ENDC,str(i), "command:", cmd,":", cmd['action'], "retcode ==", ret)
